...
# metadataAPIRequestWithToken is used to fetch instance metadata using IMDSv2
...
# metadataAPIRequestWithoutToken is used to fetch instance metadata using IMDSv1, Instance can enable it by setting
...
class AwsHypervisorManager:
    ...
    def __init__(self, access_key_id: str, secret_key_id: str, session_token: str, region: str):
    ...
    def authorize_security_group_ingress(
        ...
        Returns:
            None
        """
        resp = self.__ec2_client.authorize_security_group_ingress(
            GroupId=group_id,
            IpPermissions=ip_permissions,
        )
        if not resp.get('Return'):
            raise Exception("Security group %s ingress authorize failed: %s", group_id, resp)
    ...
    def authorize_security_group_egress(
        ...
        Returns:
            None
        """
        resp = self.__ec2_client.authorize_security_group_egress(
            GroupId=group_id,
            IpPermissions=ip_permissions,
        )
        if not resp.get('Return'):
            raise Exception("Security group %s egress authorize failed: %s", group_id, resp)
    ...
    def create_cluster(
        ...
        while True:
            ...
            if resp['cluster']['status'] == 'ACTIVE' or time.time() > timeout:
                break
            time.sleep(60)  # sleep for a minute
        print(f"Created EKS cluster {name}: {resp}")

        if resp['cluster']['status'] != 'ACTIVE':
            raise Exception("Cluster status is not ACTIVE: %s", resp['cluster']['status'])

        return (resp["cluster"]["name"], resp["cluster"]["arn"],
                resp["cluster"]["endpoint"], resp["cluster"]["certificateAuthority"]["data"])

    def get_eks_node_ami(
            self,
            eks_version: str,
    ) -> str:
        ...
        resp = self.__ssm_client.get_parameter(
            Name=parameter_name,
        )
    ...
    def create_launch_template(
        ...
        Args:
            ...
            instance_profile_name: The name of IAM instance profile
    ...
    def create_autoscaling_group(
        ...
        resp = self.__autoscaling_client.create_auto_scaling_group(
        ...
        print(f"Created autoscaling group {name}: {resp}", name, resp)
...
def get_default_domain_name(region: str) -> str:
    ...
    return "%s.compute.internal" % region
...
def strip_paths_from_arn(arn: str) -> str:
    ...
    role_without_path = resource_parts[len(resource_parts)-1]
    arn_without_path = (ARN_PREFIX +
        ...
        "clusters": [
            {
                "cluster": {
                    ...
                    "certificate-authority-data": str(cluster_certificate_authority)
                },
                "name": cluster_arn
            }
        ],
...
def get_config_map(
    ...
    worker_node_role_arn_without_path = strip_paths_from_arn(worker_node_role_arn)
    cross_account_role_arn_without_path = strip_paths_from_arn(cross_account_role_arn)
    ...
    config_map = client.V1ConfigMap(
        ...
        data={
            MAP_ROLES_DATA_KEY: MAP_ROLE_DATA_FORMAT % (
...
def setup_customer_exocompute() -> str:
    """Setup security groups for cluster and worker
    ...
    aws_session_id: str = "<AWS_Session_Token>"
    aws_region: str = "<Region>"
    vpc_id: str = "<VPC_ID>"
    jumpbox_security_group_id: str = "<Jump_box_Security_group_id>"
    subnet_ids: List[str] = ["<SUBNET>", "<SUBNET>"]
    prefix: str = "<Cluster_prefix>"
    ...
    # Get below values from RSC AWS onboarding
    master_role_arn = "<MASTER_ROLE_ARN>"
    worker_node_role_arn = "<WORKER_NODE_ARN>"
    worker_node_instance_profile_name = "<INSTANCE_PROFILE_NAME>" #not the arn
    cross_account_role =  "<CROSS_ACCOUNT_ROLE_ARN>"
    ...
    launch_template_id = setup_launch_template(
        ...
        region=aws_region,
        prefix=prefix,
        instance_profile_name=worker_node_instance_profile_name,
